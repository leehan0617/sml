	function viewbasketBallBoard(root,sportCode,currentPage){
		var addr = root+"/basketBallviewbasketBallBoard?sportCode="+sportCode+"&currentPage="+currentPage;
		...
		$.ajax({
			...
			success:function(data){
				...
				var list = data.basketBallBoardList;
				var rs = Math.floor((currentPage-1)/blockSize);
				var startBlock = rs*blockSize+1;
				var endBlock = startBlock+blockSize-1;
				
				$(".basketBallBody").empty();
				$(".pager").empty();
				
				$.each(list,function(i,val){
					$('.basketBallBody').append('<tr><td>'+list[i].boardNumber+"</td><td><a data-toggle='modal' data-target='#modalbasketBallBoardRead' onclick=basketBallreadbasketBallBoard(\'"+root+"\','"+sportCode+"','"+currentPage+"','"+list[i].boardNumber+"')>"
				...
				if(startBlock > blockSize){
					$('.pager').append("<li><a onclick=basketBallviewbasketBallBoard(\'"+root+"\','"+sportCode+"','"+(startBlock-blockSize)+"')>"+'Previous'+"</a></li>");
				}
				
				for(var i=startBlock ; i<=endBlock ; i++){
					$('.pager').append("<li><a onclick=basketBallviewbasketBallBoard(\'"+root+"\','"+sportCode+"','"+i+"')>"+i+"</a></li>");	
				}
				
				if(endBlock < blockCount){
					$('.pager').append("<li><a onclick=basketBallviewbasketBallBoard(\'"+root+"\','"+sportCode+"','"+(startBlock+blockSize)+"')>"+'Next'+"</a></li>");
				}
	...
	function readbasketBallBoard(root, sportCode , currentPage , boardNumber){
		  $("#modalbasketBallBoard").toggle();
		   var addr = root+"/basketBallreadbasketBallBoard?sportCode="+sportCode+"&currentPage="+currentPage+"&boardNumber="+boardNumber;
		   ...
		   $.ajax({
			   ...
			   success:function(data){
				   ...
				   $("#basketBallBoardTitle").attr('value',board.boardTitle);
				   $("#basketBallBoardContent").text(board.boardContent);
			   }
		   });
	}
	...
	function modalToggle(){
	   ...
	   function showAgeChart(root,sportCode){
		   var addr = root+"/basketBallshowAgeChart?sportCode="+sportCode;
		   
		   $.ajax({
			  type:"GET",
			  url:addr,
			  success:function(data){
				  $("#basketBallLineChart").hide();
				  $("#basketBallBarChart").hide();
				  $("#basketBallPieChart").show();
				  ...
				  $.jqplot('basketBallPieChart',[[['10대',first],['20대' , second] , ['30대' , third] , ['40대' , fourth] , ['50대' , fifth] , ['기타' , others]]],
							{  
	 					seriesDefaults:{
	   ...
	   function showCityChart(root,sportCode){
		   var addr = root+"/basketBallshowCityChart?sportCode="+sportCode;
		   ...
		   $.ajax({
			  ...
			  success:function(data){
				  console.log(data);
				  $("#basketBallPieChart").hide();
				  $("#basketBallLineChart").hide();
				  $("#basketBallBarChart").show();
				  ...
				  $.jqplot('basketBallBarChart',[[['서울',first],['경기' , second] , ['강원' , third] , ['충청도' , fourth] , ['경상도' , fifth] , ['전라도' , sixth],['기타',others]]],
						  {
		   				title:'지역별 통계',
	   ...
	   function showDayChart(root,sportCode){
		   var addr = root+"/basketBallshowDayChart?sportCode="+sportCode;
		   ...
		   $.ajax({
			  ...
			  success:function(data){
				...
				$("#basketBallPieChart").hide();
				$("#basketBallBarChart").hide();
				$("#basketBallLineChart").show();
				 $.jqplot('basketBallLineChart',[[spring,summer,fall,winter]],
				  			{
							title:'연령별 통계',
							axes:{
								xaxis:{
	   ...
	   function showLeagueInfo(root,sportCode){
		   var addr = root+"/basketBallshowLeagueInfo?sportCode="+sportCode;
	   ...
	   */
	   ...
	   function showRecentMatch(root,sportCode){
		   var addr = root+'/basketBallshowRecentMatch?sportCode='+sportCode;
