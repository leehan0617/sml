	function viewSoccerBoard(root,sportCode,currentPage){
		var addr = root+"/viewBaseBallBoard?sportCode="+sportCode+"&currentPage="+currentPage;
	...
	function readSoccerBoard(root, sportCode , currentPage , boardNumber){
		  $("#modalSoccerBoard").toggle();
		   var addr = root+"/readBaseBallBoard?sportCode="+sportCode+"&currentPage="+currentPage+"&boardNumber="+boardNumber;
	...
	function modalToggle(){
	   ...
	   function showAgeChart(root,sportCode){
		   var addr = root+"/baseBallshowAgeChart?sportCode="+sportCode;
	   ...
	   function showCityChart(root,sportCode){
		   var addr = root+"/baseBallshowCityChart?sportCode="+sportCode;
	   ...
	   function showDayChart(root,sportCode){
		   var addr = root+"/baseBallshowDayChart?sportCode="+sportCode;
	   ...
	   function showLeagueInfo(root,sportCode){
		   var addr = root+"/baseBallshowLeagueInfo?sportCode="+sportCode;
	   ...
	   function leagueTable(root , sportCode,teamName){
		   var addr = root +"/baseBallleagueTable?sportCode="+sportCode;
		   $("#leagueTable").empty();
		   //alert(teamName);
		   $.ajax({
			  type:"GET",
	   ...
	   */
	   ...
	   function showRecentMatch(root,sportCode){
		   var addr = root+'/baseBallshowRecentMatch?sportCode='+sportCode;
		   ...
		   $.ajax({
			  ...
			  success:function(data){
				  ...
				  for(var i in data.recordList){  
					  $.ajax({
						 type:"GET",
						 url:root+'/baseBallfindMatchTeams?teamCode='+data.recordList[i].teamCode+'&teamCode2='+data.recordList[i].teamCode2,
						 async:false,
