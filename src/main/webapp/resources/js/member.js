...
//	alert("form"+form.sportType.value);
	if(registerModal.sportType.value==""){
		alert("종목을 체크해주세요");
		return false;
	}
}
...
function idCheck(form,root){
	...
	alert(form.teamId.value);
	var url=root+"/member/idCheck.do?teamId="+form.teamId.value;
	window.open(url,"","width=250,height=150");
}

function teamIdCheck(registerModal,root){
	if(registerModal.teamName.value==""){
		alert("아이디를 입력하시오");
		registerModal.teamName.value.focus();
		return false;
	}
	
	var url=root+"/member/idCheckName.do?teamName="+registerModal.teamName.value;
	window.open(url,"","width=250,height=150");
}
