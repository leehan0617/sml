	   ...
	   function showDayChart(root,sportCode){
		   ...
		   $.ajax({
			  ...
			  success:function(data){
				console.log(data);
				list=data.list;
				spring=0;summer=0;fall=0;winter=0;garbage=0;
				for(var i=0;i<list.length;i++){
				...
				$("#soccerLineChart").show();
				 $.jqplot('soccerLineChart',[[['봄',spring],['여름',summer],['가을',fall],['겨울',winter]]],
						  {
		   				title:'계절별 통계',
		   				series:[{renderer:$.jqplot.BarRenderer}],
		   				axes:{
		   ...
		   $.ajax({
			  ...
			  success:function(data){
				  ...
				  for(var i in data.recordList){  
					  ...
					  $("#accordion").append('<div class="panel panel-default"><div class="panel-heading" role="tab" id="'+array[i]+'"><h4 class="panel-title"><a data-toggle="collapse" data-parent="#accordion" href="#'+array2[i]+'" aria-expanded="true" aria-controls="'+array2[i]+'">'+(data.recordList[i].gameDate.year+1900)+" / "+(data.recordList[i].gameDate.month+1)+" / "+data.recordList[i].gameDate.date+"   <img src='"+root+"/img/teamImg/"+teamAEmblem+"' style='width:25px; height:25px;'/>"+teamA+"&nbsp;vs&nbsp; <img src='"+root+"/img/teamImg/"+teamBEmblem+"' style='width:25px; height:25px;'/>"+teamB+'</a></h4></div><div id="'+array2[i]+'" class="panel-collapse collapse" role="tabpanel" aria-labelledby="'+array[i]+'"><div class="panel-body">'+"경기시간:"+data.recordList[i].gameTime+"<br/>"+"경기장소:"+data.recordList[i].gamePlace+"<br/>"+'</div></div>');   
				  }
